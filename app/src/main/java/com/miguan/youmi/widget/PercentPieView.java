...
public class PercentPieView extends View {
    ...
    public PercentPieView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PercentPieView);
        centerTextSize = typedArray.getDimension(R.styleable.PercentPieView_centerTextSize, centerTextSize);
        dataTextSize = typedArray.getDimension(R.styleable.PercentPieView_dataTextSize, dataTextSize);
        circleWidth = typedArray.getDimension(R.styleable.PercentPieView_circleWidth, circleWidth);
        centerTextColor = typedArray.getColor(R.styleable.PercentPieView_centerTextColor, centerTextColor);
        dataTextColor = typedArray.getColor(R.styleable.PercentPieView_dataTextColor, dataTextColor);
        isdrawText = typedArray.getBoolean(R.styleable.PercentPieView_drawText, true);
        typedArray.recycle();
        init();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //设置扇形外接矩形
        rectF = new RectF(circleWidth / 2,
                circleWidth / 2,
                getWidth() - circleWidth / 2,
                getHeight() - circleWidth / 2);
    }
    ...
    private void calculateAndDraw(Canvas canvas) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            percent = (float) numbers[i] / (float) sum;
            //获取百分比在360中所占度数
